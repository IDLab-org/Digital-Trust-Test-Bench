...
from app.routes.test_suites.forms import DataModelFormV1, VcTestSuiteFormV1, VcPlaygroundTestSuiteForm
...
def report():
    ...
    r = requests.get(f'{current_app.config["DTT_SERVICE_URL"]}/reports/render?project_id={project_id}&workspace_id={workspace_id}')
    r = r.text
    r = r.replace("\\", "")
    return r
...
def data_model():
    ...
    if request.method == "POST":
        ...
        r = requests.post(f'{current_app.config["DTT_SERVICE_URL"]}/test-suites/data-model', json=data)
        vc_profile = r.json()
        sleep(8)
    return render_template("pages/test_suites/data_model.jinja", form=form, title="Data Model | DTT", data_to_copy=vc_example, report_url=report_url, vc_profile=vc_profile)
...
def vc_test_suite():
    ...
    if request.method == "POST":
        ...
        requests.post(f'{current_app.config["DTT_SERVICE_URL"]}/test-suites/vc-test-suite', json=data)
        sleep(8)
    return render_template("pages/test_suites/vc_test_suite.jinja", form=form, title="VC Test Suite | DTT", report_url=report_url)
...
def vc_playground_test_suite():
    ...
    if request.method == "POST":
        ...
        requests.post(f'{current_app.config["DTT_SERVICE_URL"]}/test-suites/vc-playground-test-suite', json=data)
        sleep(8)
    return render_template("pages/test_suites/vc_playground.jinja", form=form, title="Test Suites | DTT", report_url=report_url)